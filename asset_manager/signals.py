...
@receiver(signals.pre_save, sender=File)
def generate_s3_file_key(sender, instance, **kwargs):
    """
    When a file is created, will generate an S3 Key from the File model's
...
def delete_old_file_on_new_file_upload(sender, created, instance, **kwargs):
    """
    When a File Model's file is updated to point at a new file, the old one must be deleted.
    """
    if not created:
...
def delete_file(sender, instance, **kwargs):
    """
    When a File model is deleted, will delete the corresponding S3 Object
    """
    s3_utils.delete_s3_object(instance.s3_key)
